...
describe('Matrix',  () => {
	...
	describe('setItem', ()  =>{
		...
		describe('sub', ()  =>{
			var a = new Matrix([1, 2,1], [3, 4,1]);
			var b = new Matrix([1, 3, 5], [2, 1,1]);
			it('should return [[0,-1,-4], [1,3,0]] when running Matrix.sub(new Matrix([1, 2,1], [3, 4,1]),new Matrix([1, 3, 5], [2, 1,1])  ).data ',  () => {
				assert.deepEqual([[0,-1,-4], [1,3,0]],Matrix.sub(a,b).data);
			});
		});

		describe('mul', ()  =>{
			var a = new Matrix([1, 2,1], [3, 4,1]);
			var b = new Matrix([2, 4,2], [6, 8,2]);
			it('should return [[2, 4,2], [6, 8,2]] when running Matrix.mul(new Matrix([1, 2,1], [3, 4,1]),2 ).data ',  () => {
				assert.deepEqual(b.data,Matrix.mul(a,2).data);
			});
		});
