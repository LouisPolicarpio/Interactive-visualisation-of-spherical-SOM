...
class Matrix {
	...
	static random(cols, rows = 1){
		...
		return new Matrix(...tmp2);//check on the performance of creating an array
	}
	 ...
	 * sub
	 * Static method
	 * Return return a new Matrix which is the matrix 1 minus matrix 2.
	 * params: matrix, matrix
	 */
    static sub(m1, m2){
      let dim = m1.shape;
	 ...
	 * mul
	 * Static method
	 * Return return a new Matrix which is the matrix1 times the number.
	 * params: matrix, num
	 */
    static mul(m1, num){
      let dim = m1.shape;
      ...
      for (let i =0; i<dim[0]; i++){
        for (let j = 0; j<dim[1]; j++){
		  result.setItem(i,j,m1.getItem(i,j)*num);
        }
      }
      return result;
    }
